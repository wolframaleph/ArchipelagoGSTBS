...
from .gs1_defs import GSTBSInternalStringData
from .rom_utils import gbarom_bitmask, enforce_c_type_range, BitReader
...
def load_string_table(rom: 'Rom', forest) -> dict[str, GSTBSInternalStringData]:
    string_id: int = 0
    i: int = 0
    corpus: dict[str, GSTBSInternalStringData] = dict()
    while string_id < _NUM_STRINGS:
        string_ptrs: StringPtrs = StringPtrs(*struct.unpack_from(f'<II', rom.mem_view, _STRINGS_OFFSET + i * 8))
        ...
        while j < 256 and string_id < _NUM_STRINGS:
            ...
            len_bytes, = struct.unpack_from(f'<B', rom.mem_view, string_ptrs.lengths_ptr + j) # size is only 1 byte
...
def dump_string_table(data_table, file_path: 'Path') -> None:
...
def load_tree_data(rom: 'Rom') -> GSHuffmanForest:
    forest: GSHuffmanForest = GSHuffmanForest()
    huffman_ptrs: HuffmanPtrs = HuffmanPtrs(*struct.unpack_from(f'<II', rom.mem_view, _HUFFMAN_OFFSET))
    tree_offset_ptr: int = huffman_ptrs.offsets_ptr
    for tree_symbol in range(256):
        tree_offset, = struct.unpack_from(f'<H', rom.mem_view, tree_offset_ptr)
        ...
        for bit in rom.iter_bits(tree_addr, 64):  # reads the next block of 512 bits (64 bytes)
            ...
            if bit == 1:
                ...
                char = int.from_bytes(
                    rom.read_bytes(char_addr, 3), byteorder='little', signed=False
                    # struct.unpack_from(f'<BBB', rom.mem_view, char_addr), byteorder='little', signed=False
                )
                char >>= (shift % 24)
